...
9 阶段标书流水线 + 配图工具集，将招标文件转化为可提交的 Word 投标文件。

> **重要约定**：本文档描述完整标书生成流程，用户确认环节（S4 内容冻结）需人工介入，流水线在此暂停等待确认后才能继续。
>
> **文档更新约定**：每次重大修改必须同步更新 `CHANGELOG.md`（格式见该文件）。判断标准：是否影响标书输出正确性、是否新增/修改用户可见命令、是否修改 Schema 或流水线逻辑。

## 修改历史

每次重大修改必须更新 `CHANGELOG.md`，格式见文件末尾。
...
**此环节做什么**：人工确认 S3 正文内容无误后，生成 `frozen_approval.json`，将正文内容冻结，后续 S5/S6/S7/S8 不得再修改正文。

**如果不冻结直接继续会怎样**：S5 会基于 S3 正文生成插图描述，如果后续发现正文有问题再回头改，插图描述和配图都要重新做，代价很大。
...
LLM 分析正文中的插图占位符（`![图 TU-XXX ...]`），为每张插图生成 JSON 描述文件。

**产物**：`s5_illustrations/images/inputs/svg-diagrams.json`（总配置文件）+ `TU-001.json` ~ `TU-025.json`（每张图的独立描述）

**用户确认**：检查 `svg-diagrams.json` 中每张插图的描述是否合理，特别是：
- 标题和层级是否正确
- 节点文字是否与正文内容一致
- 图型类型是否适合表现该内容
...
```bash
# 先将 S5 输出转换为统一作业格式
python -m bid_tool.pipeline.stages.s6_synthesize convert

# 生成所有插图（SVG + PNG）
python -m bid_tool.illustration.svg_renderer \
    --spec "output/s5_illustrations/images/inputs/svg-diagrams.json" \
    --output "output/s5_illustrations/images/assets/svg" \
    --png
```
...
**插图字体调整**：如需调整插图字体大小，可用 `--font-scale` 参数：
```bash
python -m bid_tool.illustration.svg_renderer \
    --spec "output/s5_illustrations/images/inputs/svg-diagrams.json" \
    --output "output/s5_illustrations/images/assets/svg" \
    --png --font-scale 0.85   # 缩小 15%
```
...
| S5 | 插图描述生成 | LLM | 建议确认 | 为每张插图生成 JSON 描述 |
| S6 | 图文合成 | 自动 | 可选确认 | 合成 + 配图生成 |
...
不需要运行流水线时，也可以单独使用配图工具生成 SVG/PNG 图：

```bash
# 生成配图（SVG + PNG）
python -m bid_tool.illustration.svg_renderer \
    --spec "examples/示例_统一文档配图任务.json" \
    --output "output/my_illustrations" \
    --png
...
| 图型 | `type` 值 | 适用场景 |
|------|-----------|---------|
| 分层架构图 | `layered_architecture` | 系统架构、技术栈、部署拓扑 |
| 流程图 | `flowchart` | 业务流程、数据处理流 |
| 时序图 | `sequence_diagram` | 接口调用序列、消息传递 |
| 泳道流程图 | `swimlane_flowchart` | 跨角色/跨系统流程 |
| 能力地图 | `capability_map` | 功能矩阵、模块总览 |
| 关系图 | `relationship_map` | E-R 图、数据表关联、组件依赖 |

4 套配色主题：`clarity_blue`（默认）、`navy_teal`、`royal_gold`、`slate_emerald`

47 个内置 SVG 图标，可通过 `icon` 字段引用。
...
bid-tool/
├── README.md                       # 本文件
├── CHANGELOG.md                    # 修改历史（每次重大修改必须更新）
├── pyproject.toml                  # 包配置
├── src/bid_tool/
│   ├── cli.py                      # 统一 CLI 入口 (bid-tool init/run/status/gate/...)
│   ├── __init__.py                 # 版本号导出
│   ├── pipeline/                   # 流水线子系统
│   │   ├── engine.py              # 状态机 + 阶段调度
│   │   ├── validator.py            # Schema 门禁校验
│   │   ├── trace.py                # 需求追溯矩阵
│   │   ├── style_check.py          # AI 文风检查
│   │   └── stages/                 # S1~S9 阶段脚本
...
│   ├── illustration/               # 配图子系统
│   │   ├── toolkit.py              # 配图工具统一入口
│   │   ├── svg_renderer.py         # SVG 图型渲染（含 LAYOUT 参数）
│   │   ├── icons.py                # 47 个内置 SVG 图标
│   │   ├── themes.py               # 4 套配色主题
│   │   └── echarts_workbench/      # 离线 ECharts 工作台
│   ├── schemas/                   # 12 个 JSON Schema
│   ├── prompts/                    # 5 个 LLM 提示词模板
│   └── profiles/                   # 项目类型配置 YAML
├── examples/                      # 示例输入文件（配图任务 JSON）
├── docs/                          # 详细设计文档
└── tests/                         # pytest 测试
```
...
A: 用 `--font-scale` 参数，如 `--font-scale 1.1`（放大 10%）、`--font-scale 0.85`（缩小 15%）。默认 1.0。如果想永久调整默认大小，修改 `svg_renderer.py` 中的 `LAYOUT` 字典。

**Q: 生成的 DOCX 中插图是旧图怎么办？**

A: 删除 `output/s5_illustrations/images/assets/svg/` 和 `output/s5_illustrations/images/assets/svg/png/` 下所有旧文件，重新运行 S6 配图生成。

**Q: 对照表显示"trace.json 不可用"怎么处理？**

A: 检查 `output/trace.json` 是否存在。S1 完成时会自动生成 trace.json。如果 trace.json 字段名与 s8_export.py 期望的不匹配，s8_export.py 已做了兼容性处理（支持 `req_text`、`covered_by` 等字段）。
