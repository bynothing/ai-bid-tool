...
class HtmlCssRenderer:
    ...
    def render(self, job: IllustrationJob, items: list[IllustrationItem], context: RenderContext) -> list[RenderResult]:
        ...
        for item in items:
            try:
                ...
                print(f"  [WARN] HTML render failed for {item.id}: {exc}")
                traceback.print_exc()
                results.append(RenderResult(item.id, self.name, {}, [f"渲染失败: {exc}"]))
        return results
    ...
    def _body(self, item: IllustrationItem) -> str:
        data = item.data
        canonical = item.type
        if canonical in {"architecture.layered", "layered_architecture"}:
            return _layered_architecture(data)
        if canonical in {"capability.map", "capability_map"}:
            return _capability_map(data)
        if canonical in {"process.flowchart", "flowchart"}:
            return _flowchart_map(data)
        if canonical in {"process.swimlane", "swimlane_flowchart"}:
            return _swimlane_map(data)
        if canonical in {"relationship.domain", "relationship_map"}:
            return _relationship_map(data)
        if canonical in {"interaction.sequence", "sequence_diagram"}:
            return _sequence_map(data)
        if canonical in {"architecture.deployment", "network.topology"}:
            return _platform_cards(data)
        if canonical == "architecture.security_ring":
            return _security_ring(data)
        if canonical == "operation.incident_response":
            return _incident_response(data)
        if canonical == "timeline.gantt":
            return _gantt(data)
        if canonical == "timeline.milestone":
            return _milestones(data)
        if canonical == "matrix.risk":
            return _risk_matrix(data)
        if canonical in {"comparison.solution", "comparison.advantage"}:
            return _comparison(data)
        if canonical == "integration.interface_map":
            return _interface_map(data)
        return _generic_cards(data)
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .canvas {{ position:relative; width:1600px; min-height:600px; padding:0 42px 34px; background:#fff; overflow:hidden; }}
    .header {{ height:100px; margin:0 -42px 24px; padding:24px 52px; color:#fff; background:linear-gradient(90deg,var(--navy),#006ea8); border-bottom:4px solid #4db3d9; }}
    h1 {{ margin:0; font-size:38px; line-height:1.18; letter-spacing:0; }}
    .subtitle {{ margin-top:10px; font-size:20px; color:#e9f5ff; }}
    .doc {{ position:absolute; top:32px; right:52px; max-width:430px; text-align:right; font-size:16px; color:#d8ecfb; }}
    .note {{ margin-top:22px; padding:12px 16px; border:1px solid var(--line); border-radius:8px; color:var(--sub); font-size:16px; background:#fff; }}
    .watermark {{ position:absolute; right:68px; bottom:88px; font-size:48px; font-weight:700; color:#dcecf8; transform:rotate(-18deg); pointer-events:none; }}
    .grid {{ display:grid; gap:18px; }}
    .card {{ background:#fff; border:1px solid var(--line); border-radius:8px; box-shadow:0 2px 8px rgba(7,59,114,.06); }}
    .card-h {{ padding:16px 20px; border-bottom:1px solid var(--line); background:var(--wash); font-size:22px; font-weight:700; color:var(--navy); }}
    ...
    .layer-title {{ text-align:center; color:var(--navy); font-size:19px; font-weight:700; margin-bottom:12px; }}
    .green .layer-title {{ color:#2d6d31; }}
    .layer-items {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(92px,1fr)); gap:10px; }}
    .mini {{ min-height:58px; display:flex; align-items:center; justify-content:center; padding:6px; border:1px solid #86b7d9; border-radius:7px; text-align:center; font-size:15px; line-height:1.25; background:#fff; }}
    ...
    .req-title {{ display:flex; align-items:center; justify-content:center; padding:12px; background:#2f86bd; color:#fff; font-size:22px; font-weight:700; text-align:center; }}
    ...
    .req strong {{ display:block; color:var(--navy); font-size:18px; margin-bottom:8px; }}
    ...
    .conclusion {{ margin-top:14px; padding:12px; text-align:center; border:1px solid #9ecae1; border-radius:8px; background:#f1fbff; color:#1173a5; font-size:22px; font-weight:700; }}
    ...
    .panel-title {{ margin:-14px 0 16px; padding:12px; border-radius:0 0 6px 6px; text-align:center; color:#fff; font-size:22px; font-weight:700; background:linear-gradient(90deg,var(--blue),#2e88c2); }}
    ...
    .stage strong {{ display:block; color:var(--navy); font-size:18px; margin-bottom:8px; }}
    .stage .muted {{ font-size:15px; line-height:1.45; }}
    ...
    .domain-title {{ padding:12px 16px; background:var(--wash); color:var(--navy); font-size:20px; font-weight:700; }}
