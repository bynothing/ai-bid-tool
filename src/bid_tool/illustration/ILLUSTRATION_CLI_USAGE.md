...
> **重要**: AI 生成插图数据时，请参考 [ILLUSTRATION_DATA_SPEC.md](ILLUSTRATION_DATA_SPEC.md) 获取每种图型的完整数据 Schema 和丰富示例。数据越丰富，渲染品质越高。简单的标题+字符串列表只能产生基础布局，完整的 icon+desc+style 才能产出专业投标级图形。
...
> **生成高质量插图的唯一正确方式**: 请 AI 先阅读 [ILLUSTRATION_DATA_SPEC.md](ILLUSTRATION_DATA_SPEC.md)，该文档包含：
> - 每种图型的**完整字段说明**
> - **丰富示例**（每条 50-150 行 JSON）
> - 可用图标列表（40+）和颜色语义
> - AI 生成检查清单
>
> 核心原则：
> - 每个节点/卡片都需要 `title` + `desc` + `icon` + `style` 四要素
> - 底部添加 `requirements` 或 `safeguards` 丰富画面
> - 用 `style: "accent"` 标记核心项，`"success"` 标记完成项
> - 架构图包含 `actors`（用户）和 `integrations`（外部对接）
